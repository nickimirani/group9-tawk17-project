...
class UsersDatabase extends Database
{
    ...
    {
        $query = "INSERT INTO users (user_name, birth_year, user_password) VALUES (?, ?, ?)";

        $stmt = $this->conn->prepare($query);

        $stmt->bind_param("sss", $user->user_name, $user->birth_year, $user->user_password);
    ...
    {
        $query = "UPDATE users SET user_name=?, birth_year=?, user_password=? WHERE user_id=?;";

        $stmt = $this->conn->prepare($query);

        $stmt->bind_param("sssi", $user->user_name, $user->birth_year, $user->user_password, $customer_id);
